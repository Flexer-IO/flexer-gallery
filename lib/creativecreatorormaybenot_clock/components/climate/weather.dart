...
/// Outer wrapper that owns the icon loop controllers (needs TickerProviderStateMixin).
/// This avoids conflicting with SingleTickerProviderStateMixin used by AnimatedWidgetBaseState.
class AnimatedWeather extends StatefulWidget {
  final ClockModel model;
  final Map<ClockColor, Color> palette;

  const AnimatedWeather({Key? key, required this.model, required this.palette})
    : super(key: key);
...
class _AnimatedWeatherState extends State<AnimatedWeather>
  ...
  Widget build(BuildContext context) {
    return _AnimatedWeatherTween(
      model: widget.model,
      palette: widget.palette,
      iconLoopControllers: iconLoopControllers,
    );
  }
}

class _AnimatedWeatherTween extends ImplicitlyAnimatedWidget {
  final ClockModel model;
  final Map<ClockColor, Color> palette;
  final List<AnimationController> iconLoopControllers;

  const _AnimatedWeatherTween({
    Key? key,
    Curve curve = const ElasticInOutCurve(.91),
    Duration duration = dialRotationDuration,
    required this.model,
    required this.palette,
    required this.iconLoopControllers,
  }) : super(key: key, curve: curve, duration: duration);

  @override
  _AnimatedWeatherTweenState createState() => _AnimatedWeatherTweenState();
}

class _AnimatedWeatherTweenState
    extends AnimatedWidgetBaseState<_AnimatedWeatherTween> {
  static List<WeatherCondition> get conditions => WeatherCondition.values;
  ...
  double get _angleFromModel {
    ...
    if (newAngle.difference(oldAngle) >
        (newAngle - pi * 2).difference(oldAngle))
  ...
  void animateIcon() {
    final n = widget.iconLoopControllers[currentIndex];
  ...
  Widget weatherIcon(WeatherCondition condition, double angle) {
    final iconLoopControllers = widget.iconLoopControllers;
    switch (condition) {
...
class RenderWeather
  ...
  void describeSemanticsConfiguration(SemanticsConfiguration config) {
    ...
    config
      ...
      ..label = 'Weather condition is ${condition.name}';
  }
  ...
  void _drawBackground(Canvas canvas) {
    final fullCircleRect = Rect.fromCircle(
        ...
        shader = ui.Gradient.radial(fullCircleRect.center, _radius, [
          _backgroundHighlightColor,
          _backgroundColor,
        ]);
...
abstract class RenderWeatherIcon
    extends
        RenderCompositionChild<WeatherCondition, WeatherChildrenParentData> {
    ...
    final circleRadius = rr / 2.14,
        ...
        paint = Paint()
          ..shader = ui.Gradient.radial(
            ...
            circleRadius,
            [
              Color.lerp(_backgroundHighlightColor, _backgroundColor, 1 / 2)!,
              _backgroundColor,
            ],
            const [0.4, 1],
          ),
...
void _drawCloud(
  ...
  final h = rr / 4,
      ...
      path = Path()
        ..moveTo(0, h / 2)
        ..lineTo(-w / 2 + cr, h / 2)
        ..halfCircleTo(-w / 2 + cr * 1.2, h / 2 - cr * 1.8)
        ..quadraticBezierTo(-w / 3.6, -h / 3.2, -w / 16, h / 2 - cr * 2.7)
        ..quadraticBezierTo(w / 4, -h / 2.2, w / 2 - cr * 1.1, h / 2 - cr * 1.7)
        ..halfCircleTo(w / 2 - cr, h / 2)
        ..close();
...
class RenderFoggy extends RenderWeatherIcon {
  ...
  late TweenSequence<double> sequence;
  ...
  void _drawLine(
    ...
    final addend = Offset(
      tx * sequence.transform((animation.value + shift) % 1),
      0,
    );
  ...
  void drawCondition(Canvas canvas) {
    ...
    for (var i = 0; i < _snowflakes; i++) {
      ...
      final flakeSequence = TweenSequence<DoubleTuple>([
        ...
        TweenSequenceItem(
          tween: Tween(begin: DoubleTuple(end, 1), end: DoubleTuple(end, 0)),
          weight: 1,
        ),
        TweenSequenceItem(
      ...
      final tuple = flakeSequence.transform(
...
class RenderSunny extends RenderWeatherIcon {
  ...
  void drawCondition(Canvas canvas) {
    ...
    canvas.drawOval(
      Rect.fromCircle(center: Offset.zero, radius: radius / 9),
      paint,
    );
...
class Thunderstorm extends LeafRenderObjectWidget {
  ...
  void updateRenderObject(
    ...
    RenderThunderstorm renderObject,
  ) {
    renderObject
      ..angle = angle
      ..raindrops = raindrops
...
class RenderThunderstorm extends RenderWeatherIcon {
  ...
  void attach(PipelineOwner owner) {
    ...
    lightningSequence = TweenSequence([
      TweenSequenceItem(tween: ConstantTween<double>(1), weight: 21),
      TweenSequenceItem(
        tween: Tween<double>(
          begin: 1,
          end: .9,
      ...
      TweenSequenceItem(tween: ConstantTween<double>(1), weight: 5),
    ]);
  }
  ...
  void drawCondition(Canvas canvas) {
    ...
    _drawCloud(canvas, _cloudColor, rr, 0, -rr / 4, 1.7);
...
class RenderWindy extends RenderWeatherIcon {
  ...
  void _drawWind(
    ...
    final paint = Paint()
        ...
        paths = [
          Path()
            ..moveTo(mf * (1 - l1), -hd)
            ..lineTo(2 * mf, -hd)
            ..quadraticBezierTo(2.6 * mf, -hd, 2.6 * mf, -hd * 3)
            ..cubicTo(
              2.5 * mf,
              -hd * 6,
              1.4 * mf,
          ...
          Path()
            ..moveTo(2.542 * mf, -hd)
            ..lineTo(3 * mf, -hd)
            ..quadraticBezierTo(3.34 * mf, -hd, 3.3 * mf, -hd * 2.3)
            ..cubicTo(
              3.17 * mf,
              -hd * 3.48,
              2.77 * mf,
          ...
          Path()
            ..moveTo(mf * (1 - l2), 0)
            ..lineTo(3 * mf, 0)
            ..quadraticBezierTo(3.48 * mf, 0, 3.46 * mf, hd * 1.49)
            ..cubicTo(
              3.4 * mf,
              hd * 3.6,
              2.56 * mf,
          ...
          Path()
            ..moveTo(mf * (1 - l3), hd)
            ..lineTo(2 * mf, hd)
            ..quadraticBezierTo(2.43 * mf, hd, 2.43 * mf, hd * 2.8)
            ..cubicTo(
              2.4 * mf,
              hd * 4.8,
              1.6 * mf,
    ...
    final tuple = pathSequence.transform(
      (animation.value + animationShift) % 1,
    );
