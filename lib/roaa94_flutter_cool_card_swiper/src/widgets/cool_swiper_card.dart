...
/// Returns an [Alignment] based on the drag start position.
/// This mirrors the original utility that calculated the alignment for the
/// rotation animation. The logic approximates the original behavior:
/// - If the drag starts on the right half of the screen, the card rotates
///   around its left side (center left alignment).
/// - Otherwise, it rotates around its right side (center right alignment).
Alignment getDragStartPositionAlignment(
  ...
  final isRightSide = xPosition > screenWidth / 2;
  // Determine vertical side (top or bottom) – optional, defaults to center.
  final isTopSide = yPosition < cardHeight / 2;
