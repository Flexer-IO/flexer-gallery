...
import 'cool_swiper_card_wrapper.dart' hide Constants;
...
class CoolSwiper extends StatefulWidget {
  ...
  const CoolSwiper({
    ...
    this.initAnimationOffset = consts.Constants.initAnimationOffset,
    this.cardHeight = consts.Constants.cardHeight,
  }) : super(key: key);
...
class _CoolSwiperState extends State<CoolSwiper>
  ...
  void _onAnimationTrigger() async {
    ...
    Future.delayed(consts.Constants.backgroundCardsAnimationDuration).then(
      (_) {
        _backgroundCardsAreInFrontNotifier.value = true;
      },
    );
    Future.delayed(consts.Constants.swipeAnimationDuration).then(
      (_) {
  ...
  void initState() {
    ...
    backgroundCardsAnimationController = AnimationController(
      vsync: this,
      duration: consts.Constants.backgroundCardsAnimationDuration,
    );
  }
