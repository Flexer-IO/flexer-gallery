...
class ObstacleManager extends PositionComponent with HasGameRef<TRexGame> {
  ...
  void addNewObstacle(double speed) {
    ...
    if (duplicateObstacleCheck(type) || speed < type.multipleSpeed) {
      ...
      final obstacle = Obstacle(
        ...
        speed: speed,
        gapCoefficient:
            (gameRef.config as dynamic).gapCoefficient ?? 1.0,
      ...
      if (history.length > 1) {
        final sublist = history
            .toList()
            .sublist(0, (gameRef.config as dynamic).maxObstacleDuplication ?? 1);
  ...
  bool duplicateObstacleCheck(ObstacleType nextType) {
    ...
    return duplicateCount >=
        ((gameRef.config as dynamic).maxObstacleDuplication ?? 1);
  }
...
class Obstacle extends SpriteComponent with HasGameRef<TRexGame> {
  ...
  late List<CollisionBox> collisionBoxes = [
    for (final box in type.collisionBoxes) CollisionBox.from(box as dynamic)
  ];

  bool get isVisible => (x + width) > 0;

  // Placeholder for removal condition; currently always false to preserve original behavior.
  bool get shouldRemove => false;
