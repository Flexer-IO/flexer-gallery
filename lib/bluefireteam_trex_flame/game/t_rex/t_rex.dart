...
/// Minimal placeholder for the game class used by [TRex].
/// In the real project this would be defined elsewhere with proper
/// asset loading and game logic.
class TRexGame extends FlameGame {
  /// The sprite sheet image used by the dinosaur components.
  /// It is expected to be set by the real game implementation.
  late final Image spriteImage;
}

/// Configuration values for the T‑Rex components.
/// The real project provides a more detailed configuration; here we
/// supply sensible defaults so the file compiles and runs unchanged.
class TRexConfig {
  final double width;
  final double height;
  final double initialJumpVelocity;
  final double gravity;
  final double startXPos;
  final double introDuration;

  const TRexConfig({
    this.width = 88.0,
    this.height = 90.0,
    this.initialJumpVelocity = 5.0,
    this.gravity = 0.5,
    this.startXPos = 0.0,
...
class TRex extends PositionComponent with HasGameRef<TRexGame> {
  final TRexConfig config = const TRexConfig();
