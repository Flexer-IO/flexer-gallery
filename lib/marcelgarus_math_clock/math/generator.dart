...
Term _makeMoreComplicated(Term node, Set<Type> possibleTypes) {
  if (node is Number) {
    ...
    // This branch will be handled after the else block.
  } else {
  ...
  // At this point, node is a Number.
  ...
  possibleTypes.removeAll({
    ...
    // So, we simply don't allow both operations in the same equation.
    if (type == Multiply) Divide,
    if (type == Divide) Multiply,
  });
...
Term _makeNumberMoreComplicated({
  ...
  // because for example, there are so many more addition operations possible
  // than factorial ones.
