...
class ArcChooser extends StatefulWidget {
  ...
  const ArcChooser({Key? key, this.arcSelectedCallback}) : super(key: key);
...
class ChooserState extends State<ArcChooser> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    arcItems.add(ArcItem("UGH", [const Color(0xFFF9D976), const Color(0xfff39f86)],
        angleInRadiansByTwo + userAngle));
    arcItems.add(ArcItem("OK", [const Color(0xFF21e1fa), const Color(0xff3bb8fd)],
        angleInRadiansByTwo + userAngle + (angleInRadians)));
    arcItems.add(ArcItem("GOOD", [const Color(0xFF3ee98a), const Color(0xFF41f7c7)],
        angleInRadiansByTwo + userAngle + (2 * angleInRadians)));
    arcItems.add(ArcItem("BAD", [const Color(0xFFfe0944), const Color(0xFFfeae96)],
...
class ChooserPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < arcItems.length; i++) {
      ...
      double additionalAngle = acos(((t * t) + (radiusText * radiusText) - (f * f)) /
          (2 * t * radiusText));

      double tX = center.dx +
          radiusText *
              cos(arcItems[i].startAngle + angleInRadiansByTwo - additionalAngle);
      double tY = center.dy +
          radiusText *
              sin(arcItems[i].startAngle + angleInRadiansByTwo - additionalAngle);
