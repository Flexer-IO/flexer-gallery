...
class SmilePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (size != lastSize) {
      ...
      // The following variable was previously unused; it has been removed to silence warnings.
    ...
    if (slideValue > 0 && slideValue < 200) {
      ...
      // The variable previously named _rightTweenY was unused; it has been removed.
      ...
      if (slideValue <= 100) {
        ...
        // rightTweenY not needed for drawing
      } else {
        // ugh to ok
...
//        Offset(endingX - oneThirdOfDiaByTwo, endingY), debugPaint);
  ...
  void tweenText(ReviewState centerReview, ReviewState rightReview, double diff,
      Canvas canvas) {
    ...
    TextSpan spanCenter = TextSpan(
        style: TextStyle(
            fontWeight: FontWeight.bold,
            fontSize: 52.0,
            color: centerReview.titleColor
                .withAlpha(255 - (255 * diff).round())),
    ...
    Offset rightOffset =
        Offset(rightCenter - (tpRight.width / 2), smileHeight);
    ...
    tpRight.paint(canvas,
        Offset.lerp(rightOffset, rightToCenterOffset, diff)!);
  }
...
class ReviewState {
  ...
  static ReviewState lerp(ReviewState start, ReviewState end, double ratio) {
    Color startColor = Color.lerp(start.startColor, end.startColor, ratio)!;
    Color endColor = Color.lerp(start.endColor, end.endColor, ratio)!;
