...
class FloatingPointView extends CustomPainter {
  ...
  FloatingPointView(this._points, this._containerSize,
      {this.radius = 0, this.paddingTop = 0, this.paddingHorizon = 0}) {
    pointPainter = Paint();
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Point.remained.forEach((c) {
      if (c.isEnable) {
        _actionRemained(c);
        canvas.drawCircle(Offset(c.x, c.y), c.r,
            pointPainter..color = c.color);
      }
    });

    if (radius >= 0 && radius < waterRadians) {
    ...
    _points.forEach((c) {
      ...
      if (newPoint.isEnable) {
        Rect rect = Offset(newPoint.x - 5, newPoint.y - 5) & const Size(10, 10);
        ...
        canvas.drawCircle(Offset(newPoint.x, newPoint.y), newPoint.r,
            pointPainter);
      }
    });
  }
  ...
  Point _fixPointByRadius(Point point) {
    if (radius >= (waterRadians) && radius < math.pi) {
      ...
      point.y = (originY - paddingTop) *
          math.Random().nextInt(120).toDouble() /
          100;
  ...
  Point _updatePoint(Point _point, Size size) {
    ...
    var opacity = math.max(
        0.8 * (radius - math.pi / 2) / (3 * math.pi / 2), 0.0);
    if (opacity >= 0.8) {
      opacity = 0.1;
      _point.dA = 0.1;
    }
