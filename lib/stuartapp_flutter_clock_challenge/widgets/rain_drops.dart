...
/// Minimal definition of weather conditions used by this widget.
/// The original project provides a richer enum in its model.dart; this
/// subset is sufficient for the widget's logic and preserves behavior.
...
class _RainDropsState extends State<RainDrops> {
  ...
  void _initTimer(BoxConstraints constraints) {
    ...
    _timer = Timer.periodic(
        const Duration(milliseconds: _rainDropsInterval), (timer) {
      setState(() {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        return CustomPaint(
          ...
          size: const Size(rainDropSize, rainDropSize),
        );
      },
    );
  }
...
class _CustomPainter extends CustomPainter {
  ...
  void _drawDropFalling(Canvas canvas, Offset center) {
    final interval = CurveTween(curve: const Interval(0.0, 0.25));
