...
import '../day_night_time_picker.dart';
...
// Import state container with alias for enum access
import 'state/state_container.dart' as state;
// Import same file without the conflicting TimePickerInterval name
import 'state/state_container.dart' hide TimePickerInterval;
...
class _DayNightTimePickerIosState extends State<DayNightTimePickerIos> {
  ...
  late final TimeModelBindingState timeState;
  ...
  void init() {
    ...
    // Resolve minute interval safely
    final TimePickerInterval? minuteInterval = timeState.widget.minuteInterval is TimePickerInterval
        ? timeState.widget.minuteInterval as TimePickerInterval
        : null;
    // Resolve second interval safely
    final TimePickerInterval? secondInterval = timeState.widget.secondInterval is TimePickerInterval
        ? timeState.widget.secondInterval as TimePickerInterval
        : null;

    double minMinute = getMin(timeState.widget.minMinute, minuteInterval);
    double maxMinute = getMax(timeState.widget.maxMinute, minuteInterval);

    double minSecond = getMin(timeState.widget.minSecond, secondInterval);
    double maxSecond = getMax(timeState.widget.maxSecond, secondInterval);

    int minDiff = (maxMinute - minMinute).round();
    int secDiff = (maxSecond - minSecond).round();

    final minuteDiv = getDivisions(
        minDiff, minuteInterval);
    ...
    final secondDiv = getDivisions(
        secDiff, secondInterval);
        ...
        FixedExtentScrollController(initialItem: _hours.indexOf(h))
          ..addListener(() {
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                timeState.onSelectedInputChange(state.SelectedInput.HOUR);
              }
            });
          })
          ..addListener(() {
