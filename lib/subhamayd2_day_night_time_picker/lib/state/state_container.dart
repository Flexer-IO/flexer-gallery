...
// ---------------------------------------------------------------------------
// Stub definitions for missing types that were originally imported from
// '../day_night_time_picker.dart'. These definitions are minimal and only
// provide the members used within this file, ensuring null‑safety and Dart 3
// compatibility without altering any UI or rendering logic.
// ---------------------------------------------------------------------------

/// Represents the selectable input fields in the picker.
enum SelectedInput { HOUR, MINUTE, SECOND }

/// Interval steps for minute/second selectors.
enum TimePickerInterval { five, ten, fifteen, thirty, sixty }

/// Simple time model used by the picker.
class Time {
  final int hour;
  final int minute;
  final int second;
  ...
  ///
  /// If [period] is omitted, it is derived from [hour] (>=12 → [DayPeriod.pm],
  /// otherwise [DayPeriod.am]).
  const Time({
    required this.hour,
    required this.minute,
    required this.second,
    DayPeriod? period,
  }) : period = period ?? (hour >= 12 ? DayPeriod.pm : DayPeriod.am);

  /// Returns a new [Time] with the given [period].
  Time setPeriod(DayPeriod newPeriod) => Time(
        hour: hour,
        minute: minute,
        second: second,
        period: newPeriod,
  ...
  /// Returns a copy of this [Time] with the supplied fields replaced.
  Time replacing({int? hour, int? minute, int? second}) => Time(
        hour: hour ?? this.hour,
...
class TimeModelBindingState extends State<TimeModelBinding> {
  ...
  void initState() {
    ...
    if (widget.focusMinutePicker || (widget.disableHour ?? false)) {
  ...
  bool checkIfWithinRange(DayPeriod other) {
    if (widget.minHour == null || widget.maxHour == null) {
      return true;
    }
    final tempTime = Time(
      hour: time.hour,
