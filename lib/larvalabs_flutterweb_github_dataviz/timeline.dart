...
class TimelineState extends State<Timeline> {
  final HashMap<String, TextPainter> labelPainters = HashMap<String, TextPainter>();
  ...
  TextPainter _makeTextPainter(Color color, String label) {
    TextSpan span = TextSpan(style: TextStyle(color: color, fontSize: 12), text: label);
  ...
  double _getClampedXFractionLocalCoords(BuildContext context, Offset globalOffset) {
    ...
    return MathUtils.clamp(localOffset.dx / box.size.width, 0, 1);
  }
}

class TimelinePainter extends CustomPainter {
  final HashMap<String, TextPainter> labelPainters;

  late final Paint mainLinePaint;
  late final Paint milestoneLinePaint;

  Color lineColor = Colors.white;

  final int numWeeks;
  final double animationValue;
  final int weekYearOffset = 9; // Week 0 in our data is 9 weeks before the year boundary (i.e. week 43)
  ...
  TimelinePainter({
    ...
    mainLinePaint = Paint()
      ...
      ..color = Constants.timelineLineColor;
    milestoneLinePaint = Paint()
      ..style = PaintingStyle.stroke
      ..color = Constants.milestoneTimelineColor;
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(0, mainLineY), Offset(size.width, mainLineY), mainLinePaint);

    double currTimeX = size.width * animationValue;
    canvas.drawLine(Offset(currTimeX, labelHeightDoubled),
        Offset(currTimeX, size.height - labelHeightDoubled), milestoneLinePaint);
      ...
      for (int week = 0; week < numWeeks; week++) {
        ...
        if (lineHeight > 0) {
          ...
          if (currTimeXDiff > 0) {
            ...
            mainLinePaint.color = lerpedColor ?? Constants.timelineLineColor;
          } else {
            mainLinePaint.color = Constants.timelineLineColor;
          }
          canvas.drawLine(Offset(currX, margin), Offset(currX, size.height - margin), mainLinePaint);
        }

        if (isYear) {
          var yearLabel = "$yearNumber";
          labelPainters[yearLabel]!.paint(canvas, Offset(currX, size.height - labelHeight));
          yearNumber++;
        }
      }
    }

    {
      for (int i = 0; i < weekLabels.length; i++) {
        ...
        if (timelineXDiff > 0 && timelineXDiff < maxTimelineDiff && animationValue < 1) {
          ...
          milestoneLinePaint.color = lerpedColor ?? Constants.milestoneTimelineColor;
        } else {
        ...
        double margin = (size.height - lineHeight) / 2;
        canvas.drawLine(Offset(currX, margin), Offset(currX, size.height - margin), milestoneLinePaint);

        textPainter.paint(canvas, Offset(currX, size.height - labelHeightDoubled));
      }
    }
  }
