...
import '../controller/service_locator.dart';
import '../elements/elements.dart';
import '../elements/jardinain.dart';
import '../elements/obstacle.dart';
...
class _GameViewState extends State<GameView> {
  ...
  final double fps = 1 / 60; // Frame Rate of 60 Frames per second
  final PlayBall playBall = PlayBall();
  Size screenSize = Size.zero,
      brickWallSize = Size.zero,
      platformSize = const Size(200, 50),
      brickSize = const Size(70, 25);
  ...
  void frameBuilder(dynamic timestamp) {
    jardinainFall();
    obstacleFall();
  ...
  void createBrickWall() {
    bricks = [];
    int maxColumnCount =
        (brickWallSize.width / brickSize.width).floor();
    int maxRowCount =
        (brickWallSize.height / brickSize.height).floor();
    bool prevOdd = true; // Offset the Alignment of Bricks to mimic a real wall
    for (int x = 0; x < maxRowCount; x++) {
      int currentColumnCount = max(
          x == 0 ? maxColumnCount : 4,
          random.nextInt(maxColumnCount));
      ...
      double leftOffset = ((screenSize.width -
              (currentColumnCount * brickSize.width)) /
          2);
      double previousLocation = 0;
      List<Jardinain> jardinains = [];
      for (int y = 0; y < currentColumnCount; y++) {
        Rect rect = Rect.fromLTWH(
            leftOffset + (y.toDouble() * brickSize.width),
            _jardinainController.jardinainHeight +
                x * brickSize.height,
            brickSize.width,
            brickSize.height);
      ...
      return GestureDetector(
        onTap: () {
          if (!launched && !finishing) {
            launched = true;
            playBall.position =
                PVector(pointer.dx, screenSize.height - platformSize.height - playBall.radius);
            playBall.velocity =
                PVector(-mode.toDouble(), -mode.toDouble());
        ...
        onPanUpdate: (panDetails) {
          if (isMobile) {
            pointer = panDetails.localPosition;
          }
        },
        child: Center(
          child: Container(
                        ...
                        child: const Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              // Placeholder for health icons; actual icons are added at runtime
                            ],
                          ),
                        ),
                      )),
                  if (finishing) ...[
                    Center(
                      child: bricks.isEmpty
                          ? Image.asset("assets/images/up_high.gif")
                          : Image.asset("assets/images/laughing.gif"),
                    )
                  ],
                  if (reviving) ...[
                    Center(
