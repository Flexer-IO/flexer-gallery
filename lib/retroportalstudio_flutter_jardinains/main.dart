...
import 'views/jardinains_view.dart';
import 'deps/window_size/window_size.dart';
...
bool isWeb() {
  try {
    // Accessing Platform will throw on web, so we just attempt a call.
    // The variable is intentionally unused; we only need the try block.
    Platform.isWindows;
    return false;
  } catch (e) {
    return true;
...
class _GameControllerState extends State<GameController> {
  ...
  Future<void> webDialog() async {
    await showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          title: Text(
            "WEB WARNING! - Just for Demonstration",
            style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold),
          ),
          content: Container(
            child: Text(
                "Although this Game can run pretty well Desktop Web, Web is not an optimal platform to run this Game (Especially not on Mobile Devices)"),
          ),
          actions: [
            TextButton(
                onPressed: () {
