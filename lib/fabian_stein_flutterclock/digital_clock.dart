...
// ---------------------------------------------------------------------------
// Stub implementation of ClockModel for null‑safety and to resolve the missing
// import. The real implementation (from flutter_clock_helper) provides the same
// interface, so this stub is sufficient for compilation and runtime behaviour
// required by this widget.
// ---------------------------------------------------------------------------
class ClockModel extends ChangeNotifier {
  /// Whether the clock should use a 24‑hour format.
  bool is24HourFormat = true;

  // The real model may expose more properties, but the widget only uses
  // [is24HourFormat] and the listener registration methods inherited from
  // ChangeNotifier.
}
...
List<Node> nodes = List<Node>.filled(2500, Node(false), growable: false);
...
List<QuadrantDescriptor> quadrantDescriptors =
    List<QuadrantDescriptor>.generate(quadrants(), (_) => QuadrantDescriptor());
...
class Painter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
...

    {
      ...
      List<int> indexs = List<int>.filled(nodes.length, 0, growable: false);
    ...
    double x, y;
    double tangensAngle = atan((size.width / 2.0) / (size.height / 2.0));
      ...
      if (node.checkCoordinate(_hour, _minute, _drawInside)) {
        ...
        */
        ...
        for (var index in quadrantsToCheck) {
          ...
          int startingIndex = (quadrantDescriptor.beginningIndex > node.index
                  ? quadrantDescriptor.beginningIndex
                  : node.index);
          ...
          for (int i = startingIndex;
            ...
            double distance = point1.distanceTo(point2).toDouble();
            ...
            if (node2.checkCoordinate(_hour, _minute, _drawInside)) {
              ...
              final double maxDist = maxDistanceBetweenNodes.toDouble();
              if (distance < maxDist) {
                ...
                double alpha = min(0.015 / (distance + 0.0001), 1.0).toDouble();
