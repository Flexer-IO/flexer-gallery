...
// Define the GitHub repository URL if it's not imported from elsewhere.
const String kGithubRepo = 'https://github.com/JideGuru/FlutterLetsDraw';

/// Extension to provide missing members for [UndoRedoStack] when they are
/// not defined in the original implementation. This ensures null‑safety and
/// compatibility with Dart 3 without altering UI behavior.
extension UndoRedoStackExtensions on UndoRedoStack {
  /// A dummy [ValueNotifier] that always reports `false`. If the original
  /// implementation provides its own `canRedo` as a [ValueListenable],
  /// this getter will be ignored because the member already exists.
  ValueNotifier<bool> get canRedo => ValueNotifier<bool>(false);

  /// A no‑op clear method. If the original implementation already defines a
  /// `clear` method, this extension will not be used.
  void clear() {}
}
