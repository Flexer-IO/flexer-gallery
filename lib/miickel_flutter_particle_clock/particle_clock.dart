...
class ParticleClock extends StatefulWidget {
  const ParticleClock(this.model, {super.key});
...
class _ParticleClockState extends State<ParticleClock> with SingleTickerProviderStateMixin {
  ...
  Future<List<Palette>> _loadPalettes() async {
    String data = await DefaultAssetBundle.of(context).loadString("assets/miickel_flutter_particle_clock/palettes.json");
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Container(
            child: Center(
              child: const Text("Could not load palettes."),
            ),
          );
        }
